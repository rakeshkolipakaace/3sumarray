...
bool runTest(int testNumber, vector<int> nums, vector<vector<int>> expected) {
    Solution solution;
    vector<vector<int>> result = solution.threeSum(nums);
    ...
    bool passed = (result == expected);
    cout << "Test " << testNumber << ": " << (passed ? "✅ PASSED" : "❌ FAILED") << "\n";
    if (!passed) {
        cout << "Expected: ";
        printVectorOfVectors(expected);
        cout << "Got: ";
...
int main() {
    vector<pair<vector<int>, vector<vector<int>>>> testCases = {
        ...
        {{-4, -1, -1, 0, 1, 2, 3}, {{-4, 1, 3}, {-1, -1, 2}, {-1, 0, 1}}},
        {{-3, -2, -1, 0, 1, 2, 3}, {{-3, 0, 3}, {-3, 1, 2}, {-2, -1, 3}, {-2, 0, 2}, {-1, 0, 1}}},
        {{-10, -3, 0, 1, 2, 3, 10}, {{-10, 0, 10}, {-3, 0, 3}, {-3, 1, 2}}},
        {{-5, -1, -1, 0, 1, 1, 2, 3, 4}, {{-5, 1, 4}, {-5, 2, 3}, {-1, -1, 2}, {-1, 0, 1}}}
    };

    int passedCount = 0;
    for (size_t i = 0; i < testCases.size(); i++) {
        if (runTest(i + 1, testCases[i].first, testCases[i].second)) {
            passedCount++;
        }
    }

    cout << "Summary: " << passedCount << "/" << testCases.size() << " tests passed\n";
